Meteor.methods
	...
	push_test: ->
		...
		if not CaoLiao.authz.hasRole(user._id, 'admin')
			throw new Meteor.Error 'error-not-allowed', 'Not allowed', { method: 'push_test' }
...
configurePush = ->
	if CaoLiao.settings.get 'Push_debug'
		console.log 'Push: configuring...'

	if CaoLiao.settings.get('Push_enable') is true
		Push.allow
			send: (userId, notification) ->
				return CaoLiao.authz.hasRole(userId, 'admin')

		apn = undefined
		gcm = undefined

		if CaoLiao.settings.get('Push_enable_gateway') is false
			gcm =
				apiKey: CaoLiao.settings.get 'Push_gcm_api_key'
				projectNumber: CaoLiao.settings.get 'Push_gcm_project_number'

			apn =
				passphrase: CaoLiao.settings.get 'Push_apn_passphrase'
				keyData: CaoLiao.settings.get 'Push_apn_key'
				certData: CaoLiao.settings.get 'Push_apn_cert'

			if CaoLiao.settings.get('Push_production') isnt true
				apn =
					passphrase: CaoLiao.settings.get 'Push_apn_dev_passphrase'
					keyData: CaoLiao.settings.get 'Push_apn_dev_key'
					certData: CaoLiao.settings.get 'Push_apn_dev_cert'
		...
		Push.Configure
			apn: apn
			gcm: gcm
			production: CaoLiao.settings.get 'Push_production'
			sendInterval: 1000
			sendBatchSize: 10

		if CaoLiao.settings.get('Push_enable_gateway') is true
...
sendPush = (service, token, options, tries=0) ->
	...
	HTTP.post CaoLiao.settings.get('Push_gateway') + "/push/#{service}/send", data, (error, response) ->
