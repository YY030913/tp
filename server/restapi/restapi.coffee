...
Api.addRoute 'version', authRequired: false,
	get: ->
		version = {api: '0.1', caoliao: '0.5'}
		status: 'success', versions: version

Api.addRoute 'publicRooms', authRequired: true,
	get: ->
		rooms = CaoLiao.models.Rooms.findByType('c', { sort: { msgs:-1 } }).fetch()
	...
	get: ->
		try
			if Meteor.call('canAccessRoom', @urlParams.id, this.userId)
				msgs = CaoLiao.models.Messages.findVisibleByRoomId(@urlParams.id, {sort: {ts: -1}, limit: 50}).fetch()
...
Api.addRoute 'rooms/:id/online', authRequired: true,
	get: ->
		room = CaoLiao.models.Rooms.findOneById @urlParams.id
		online = CaoLiao.models.Users.findUsersNotOffline(fields:
			username: 1
			status: 1).fetch()
		onlineInRoom = []
	...
	for user, i in users
		if user.name?
			if user.email?
				if user.pass?
					try
						nameValidation = new RegExp '^' + CaoLiao.settings.get('UTF8_Names_Validation') + '$', 'i'
					catch
...
Api.addRoute 'bulk/register', authRequired: true,
	post:
		...
		action: ->
			if CaoLiao.authz.hasPermission(@userId, 'bulk-register-user')
				try
...
Api.addRoute 'bulk/createRoom', authRequired: true,
	post:
		...
		action: ->
			...
			if CaoLiao.authz.hasPermission(@userId, 'bulk-create-c')
				try
