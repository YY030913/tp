Meteor.methods
	updateMessage: (message) ->
		...
		originalMessage = CaoLiao.models.Messages.findOneById message._id

		if not originalMessage?._id?
			return

		hasPermission = CaoLiao.authz.hasPermission(Meteor.userId(), 'edit-message', message.rid)
		editAllowed = CaoLiao.settings.get 'Message_AllowEditing'
		editOwn = originalMessage?.u?._id is Meteor.userId()

		me = CaoLiao.models.Users.findOneById Meteor.userId()
		...
		if CaoLiao.settings.get 'Message_KeepHistory'
		...
		CaoLiao.models.Messages.update
			_id: tempid
		,
			$set: message
		...
		Meteor.defer ->
			CaoLiao.callbacks.run 'afterSaveMessage', CaoLiao.models.Messages.findOneById(tempid), room
