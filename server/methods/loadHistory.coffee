Meteor.methods
	loadHistory: (rid, end, limit=20, ls) ->
		...
		if end?
			records = CaoLiao.models.Messages.findVisibleByRoomIdBeforeTimestamp(rid, end, options).fetch()
		else
			records = CaoLiao.models.Messages.findVisibleByRoomId(rid, options).fetch()
		...
		if ls?
			...
			if firstMessage?.ts > ls
				...
				unreadMessages = CaoLiao.models.Messages.findVisibleByRoomIdBetweenTimestamps(rid, ls, firstMessage.ts, { limit: 1, sort: { ts: 1 } })
