Meteor.methods
	loadSurroundingMessages: (message, limit=50) ->
		...
		message = CaoLiao.models.Messages.findOneById(message._id);
		...
		records = CaoLiao.models.Messages.findVisibleByRoomIdBeforeTimestamp(message.rid, message.ts, options).fetch()
		...
		records = CaoLiao.models.Messages.findVisibleByRoomIdAfterTimestamp(message.rid, message.ts, options).fetch()
