Meteor.methods
	addUserToRoom: (data) ->
		...
		if not CaoLiao.authz.hasPermission(fromId, 'add-user-to-room', room._id)
			throw new Meteor.Error 'error-not-allowed', 'Not allowed', { method: 'addUserToRoom' }
		...
		newUser = CaoLiao.models.Users.findOneByUsername data.username

		CaoLiao.models.Rooms.addUsernameById data.rid, data.username

		now = new Date()

		CaoLiao.models.Subscriptions.createWithRoomAndUser room, newUser,
			ts: now
			open: true
			alert: true
			unread: 1
		...
		CaoLiao.models.Messages.createUserAddedWithRoomIdAndUser data.rid, newUser,
			ts: now
			u:
				_id: fromUser._id
