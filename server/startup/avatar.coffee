Meteor.startup ->
	...
	if CaoLiao.settings.get 'Accounts_AvatarStoreType'
		storeType = CaoLiao.settings.get 'Accounts_AvatarStoreType'

	CaoLiaoStore = CaoLiaoFile[storeType]

	if not CaoLiaoStore?
		throw new Error "Invalid CaoLiaoStore type [#{storeType}]"

	console.log "Using #{storeType} for Avatar storage".green

	transformWrite = (file, readStream, writeStream) ->
		if CaoLiaoFile.enabled is false or CaoLiao.settings.get('Accounts_AvatarResize') isnt true
			return readStream.pipe writeStream

		height = CaoLiao.settings.get 'Accounts_AvatarSize'
		width = height

		CaoLiaoFile.gm(readStream, file.fileName).background('#ffffff').resize(width, height+'^>').gravity('Center').extent(width, height).stream('jpeg').pipe(writeStream)

	path = "~/uploads"

	if CaoLiao.settings.get('Accounts_AvatarStorePath')?.trim() isnt ''
		path = CaoLiao.settings.get 'Accounts_AvatarStorePath'

	@CaoLiaoFileAvatarInstance = new CaoLiaoStore
		name: 'avatars'
		absolutePath: path
	...
	WebApp.connectHandlers.use '/avatar/', Meteor.bindEnvironment (req, res, next) ->
		...
		if params.username[0] isnt '@'
			if Meteor.settings?.public?.sandstorm
				user = CaoLiao.models.Users.findOneByUsername(params.username.replace('.jpg', ''))
				if user?.services?.sandstorm?.picture
					res.setHeader 'Location', user.services.sandstorm.picture
					res.writeHead 302
					res.end()
					return
			file = CaoLiaoFileAvatarInstance.getFileWithReadStream encodeURIComponent(params.username)
		else
