CaoLiao.Migrations.add
	...
	up: ->
		...
		newFilesCollection  = CaoLiao.models.Uploads
		newGridFSCollection = new Meteor.Collection 'caoliao_uploads.files'
		newChunkCollection  = new Meteor.Collection 'caoliao_uploads.chunks'


		oldFilesCollection.find({'copies.files.key': {$exists: true}}).forEach (cfsRecord) ->
			nameParts = cfsRecord.original.name?.split('.')
			extension = ''
			url = "ufs/caoliao_uploads/#{cfsRecord._id}"
			...
			CaoLiao.models.Messages.find({$or: [{ 'urls.url': "https://demo.caoliao/cfs/files/Files/#{cfsRecord._id}" }, { 'urls.url': "https://caoliao/cfs/files/Files/#{cfsRecord._id}" }]}).forEach (message) ->
				for urlsItem in message.urls
					if urlsItem.url is "https://demo.caoliao/cfs/files/Files/#{cfsRecord._id}" or urlsItem.url is "https://caoliao/cfs/files/Files/#{cfsRecord._id}"
						urlsItem.url = Meteor.absoluteUrl() + url
						if urlsItem.parsedUrl?.pathname?
							urlsItem.parsedUrl.pathname = "/#{url}"
						message.msg = message.msg.replace "https://demo.caoliao/cfs/files/Files/#{cfsRecord._id}", Meteor.absoluteUrl() + url
						message.msg = message.msg.replace "https://caoliao/cfs/files/Files/#{cfsRecord._id}", Meteor.absoluteUrl() + url

				CaoLiao.models.Messages.update {_id: message._id}, {$set: {urls: message.urls, msg: message.msg}}
