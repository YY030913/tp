CaoLiao.Migrations.add({
	version: 40,
	up: function() {
		CaoLiao.models.Settings.find({ _id: /Accounts_OAuth_Custom_/, i18nLabel: 'Accounts_OAuth_Custom_Enable' }).forEach(function(customOauth) {
			var parts = customOauth._id.split('_');
			var name = parts[3];
			var id = 'Accounts_OAuth_Custom_' + name + '_token_sent_via';
			if (!CaoLiao.models.Settings.findOne({ _id: id })) {
				CaoLiao.models.Settings.insert({
					'_id': id,
					'type': 'select',
					'group': 'OAuth',
