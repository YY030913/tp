Meteor.publish 'spotlight', (selector, options, collName) ->
	...
	subHandleUsers = CaoLiao.models.Users.findUsersByNameOrUsername(new RegExp(selector.name.$regex, 'i'), { limit: 10, fields: { name: 1, username: 1, status: 1 }, sort: { name: 1 } }).observeChanges
		added: (id, fields) ->
	...
	subHandleRooms = CaoLiao.models.Rooms.findByNameContainingAndTypes(selector.name.$regex, ['c'], { limit: 10, fields: { t: 1, name: 1 }, sort: {name: 1}}).observeChanges
		added: (id, fields) ->
