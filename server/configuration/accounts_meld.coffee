...
Accounts.updateOrCreateUserFromExternalService = (serviceName, serviceData, options) ->
	...
	if serviceData.email
		...
		user = CaoLiao.models.Users.findOneByEmailAddress serviceData.email
		if user?
			...
			if not _.findWhere user.emails, { address: serviceData.email, verified: true }
				CaoLiao.models.Users.resetPasswordAndSetRequirePasswordChange(user._id, true, 'This_email_has_already_been_used_and_has_not_been_verified__Please_change_your_password')

			# Merge accounts
			CaoLiao.models.Users.setServiceId user._id, serviceName, serviceData.id

			# Validate email
			CaoLiao.models.Users.setEmailVerified user._id, serviceData.email
