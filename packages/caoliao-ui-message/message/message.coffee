Template.message.helpers
	...
	roleTags: ->
		unless CaoLiao.settings.get('UI_DisplayRoles')
			return []
		roles = _.union(UserRoles.findOne(this.u?._id)?.roles, RoomRoles.findOne({'u._id': this.u?._id, rid: this.rid })?.roles)
		return _.compact(_.map(roles, (role) -> return CaoLiao.models.Roles.findOne({ _id: role, description: { $exists: 1 } })?.description));
	isGroupable: ->
	...
	timestamp: ->
		return +this.ts
	chatops: ->
		return 'chatops-message' if this.u?.username is CaoLiao.settings.get('Chatops_Username')
	time: ->
		return moment(this.ts).format(CaoLiao.settings.get('Message_TimeFormat'))
	date: ->
		return moment(this.ts).format(CaoLiao.settings.get('Message_DateFormat'))
	isTemp: ->
		if @temp is true
			return 'temp'
	body: ->
	...
	system: ->
		if CaoLiao.MessageTypes.isSystemMessage(this)
			return 'system'
	...
	canEdit: ->
		hasPermission = CaoLiao.authz.hasAtLeastOnePermission('edit-message', this.rid)
	...
	canDelete: ->
		hasPermission = CaoLiao.authz.hasAtLeastOnePermission('delete-message', this.rid )
	...
	showEditedStatus: ->
		return CaoLiao.settings.get 'Message_ShowEditedStatus'
	label: ->
		if @i18nLabel
			return t(@i18nLabel)
		else if @label
	...
	hasOembed: ->
		return false unless this.urls?.length > 0 and Template.oembedBaseWidget? and CaoLiao.settings.get 'API_Embed'

		return false unless this.u?.username not in CaoLiao.settings.get('API_EmbedDisabledFor')?.split(',')
	...
	hideCog: ->
		room = CaoLiao.models.Rooms.findOne({ _id: this.rid });
...
Template.message.onCreated ->
	...
	@msgtype = msg.t or "u"
	@wasEdited = msg.editedAt? and not CaoLiao.MessageTypes.isSystemMessage(msg)

	@tipoff = do ->
		isSystemMessage = CaoLiao.MessageTypes.isSystemMessage(msg)
	...
	@body = do ->
		...
		if messageType?.render?
			...
			if msg.u?.username is CaoLiao.settings.get('Chatops_Username')
				msg.html = msg.msg
				msg = CaoLiao.callbacks.run 'renderMentions', msg
...
Template.message.onViewRendered = (context) ->
	...
	# Meteor.defer ->
	if $(".attachment-title")?.width() < 300
		$("audio").width($(".attachment-title").width())
	...
	this._domrange.onAttached (domRange) ->
		...
		unless previousNode?
			...
			if currentDataset.timestamp - previousDataset.timestamp > 300000
				$currentNode.addClass('new-day').removeClass('sequential')
			else
			...
			if previousDataset.groupable is 'false' or currentDataset.groupable is 'false'
				...
				if previousDataset.username isnt currentDataset.username or parseInt(currentDataset.timestamp) - parseInt(previousDataset.timestamp) > CaoLiao.settings.get('Message_GroupingPeriod') * 1000
					$currentNode.removeClass('sequential')
		...
		if nextNode?.dataset?
			...
			if nextDataset.timestamp - currentDataset.timestamp > 300000
				$currentNode.addClass('new-day').removeClass('sequential')
			else
				$currentNode.removeClass('new-day')
			...
			if nextDataset.groupable isnt 'false'
				if nextDataset.username isnt currentDataset.username or parseInt(nextDataset.timestamp) - parseInt(currentDataset.timestamp) > CaoLiao.settings.get('Message_GroupingPeriod') * 1000
