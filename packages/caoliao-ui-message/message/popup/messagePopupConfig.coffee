...
Meteor.startup ->
	Tracker.autorun ->
		...
		messageUsers = CaoLiao.models.Messages.find({rid: Session.get('openedRoom'), 'u.username': {$ne: Meteor.user().username}}, {fields: {'u.username': 1, ts: 1}, sort: {ts: -1}}).fetch()
...
Template.messagePopupConfig.helpers
		...
		config =
			...
			getFilter: (collection, filter) ->
				...
				if items.length < 5 and filter?.trim() isnt ''
					messageUsers = _.pluck(items, 'username')
					Tracker.nonreactive ->
						roomUsernames = CaoLiao.models.Rooms.findOne(Session.get('openedRoom')).usernames
	...
	popupSlashCommandsConfig: ->
		...
		config =
			title: t('Commands')
			collection: CaoLiao.slashCommands.commands
			trigger: '/'
		...
		if CaoLiao.emoji?
			...
			config =
				...
				getFilter: (collection, filter) ->
					...
					if CaoLiao.emoji.asciiList[key] or filter.length < 2
						return []

					# use ascii
					for shortname, value of CaoLiao.emoji.asciiList
