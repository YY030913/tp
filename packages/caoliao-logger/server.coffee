...
@Logger = class Logger
	...
	_getCallerDetails: ->
		...
		for item, index in lines when index > 0
			...
			if not line.match(/packages\/caoliao_logger(?:\/|\.js)/)
				break
...
StdOut = new class extends EventEmitter
	constructor: ->
		...
		process.stdout.write = (string, encoding, fd) =>
			...
			if CaoLiao?.settings?.get('Log_View_Limit')? and @queue.length > CaoLiao.settings.get('Log_View_Limit')
				@queue.shift()
...
Meteor.publish 'stdout', ->
	...
	if CaoLiao.authz.hasPermission(@userId, 'view-logs') isnt true
		return @ready()
