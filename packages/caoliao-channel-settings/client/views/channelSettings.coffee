Template.channelSettings.helpers
	canEdit: ->
		return CaoLiao.authz.hasAllPermission('edit-room', @rid)
	canArchiveOrUnarchive: ->
		return CaoLiao.authz.hasAtLeastOnePermission(['archive-room', 'unarchive-room'], @rid)
	editing: (field) ->
	...
	channelSettings: ->
		return CaoLiao.ChannelSettings.getOptions()
	roomTypeDescription: ->
...
Template.channelSettings.onCreated ->
	...
	@validateRoomName = =>
		...
		if not CaoLiao.authz.hasAllPermission('edit-room', rid) or room.t not in ['c', 'p']
