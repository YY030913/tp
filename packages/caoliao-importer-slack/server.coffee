Importer.Slack = class Importer.Slack extends Importer.Base
	...
	startImport: (importSelection) =>
		...
		Meteor.defer =>
			...
			for user in @users.users when user.do_import
				do (user) =>
					Meteor.runAsUser startedByUserId, () =>
						existantUser = CaoLiao.models.Users.findOneByEmailAddress user.profile.email
						if not existantUser
							existantUser = CaoLiao.models.Users.findOneByUsername user.name
						...
						if existantUser
							...
							Meteor.runAsUser userId, () =>
								...
								# Slack's is -18000 which translates to CaoLiao's after dividing by 3600
								if user.tz_offset
									Meteor.call 'updateUserUtcOffset', user.tz_offset / 3600

							if user.profile.real_name
								CaoLiao.models.Users.setName userId, user.profile.real_name
							#Deleted users are 'inactive' users in CaoLiao
							if user.deleted
			...
			for channel in @channels.channels when channel.do_import
				do (channel) =>
					Meteor.runAsUser startedByUserId, () =>
						existantRoom = CaoLiao.models.Rooms.findOneByName channel.name
						if existantRoom or channel.is_general
							...
							CaoLiao.models.Rooms.update { _id: channel.rocketId }, { $set: roomUpdate }
				...
				do (channel, messagesObj) =>
					Meteor.runAsUser startedByUserId, () =>
						...
						if slackChannel?.do_import
							room = CaoLiao.models.Rooms.findOneById slackChannel.rocketId, { fields: { usernames: 1, t: 1, name: 1 } }
							for date, msgs of messagesObj
										...
										if message.subtype?
											if message.subtype is 'channel_join'
												if @getRocketUser(message.user)?
													CaoLiao.models.Messages.createUserJoinWithRoomIdAndUser room._id, @getRocketUser(message.user), msgDataDefaults
											else if message.subtype is 'channel_leave'
												if @getRocketUser(message.user)?
													CaoLiao.models.Messages.createUserLeaveWithRoomIdAndUser room._id, @getRocketUser(message.user), msgDataDefaults
											else if message.subtype is 'me_message'
												msgObj =
													msg: "_#{@convertSlackMessageToCaoLiao(message.text)}_"
												_.extend msgObj, msgDataDefaults
												CaoLiao.sendMessage @getRocketUser(message.user), msgObj, room
											else if message.subtype is 'bot_message'
												botUser = CaoLiao.models.Users.findOneById 'caoliao', { fields: { username: 1 }}
												botUsername = if @bots[message.bot_id] then @bots[message.bot_id]?.name else message.username
												msgObj =
													msg: @convertSlackMessageToCaoLiao(message.text)
													rid: room._id
													bot: true
												...
												CaoLiao.sendMessage botUser, msgObj, room, upsert: true
											else if message.subtype is 'channel_purpose'
												CaoLiao.models.Messages.createRoomSettingsChangedWithTypeRoomIdMessageAndUser 'room_changed_topic', room._id, message.purpose, @getRocketUser(message.user), msgDataDefaults
											else if message.subtype is 'channel_topic'
												CaoLiao.models.Messages.createRoomSettingsChangedWithTypeRoomIdMessageAndUser 'room_changed_topic', room._id, message.topic, @getRocketUser(message.user), msgDataDefaults
											else if message.subtype is 'pinned_item'
												if message.attachments
													msgObj =
														attachments: [
															"text" : @convertSlackMessageToCaoLiao message.attachments[0].text
															"author_name" : message.attachments[0].author_subname
															"author_icon" : getAvatarUrlFromUsername(message.attachments[0].author_subname)
														]
													_.extend msgObj, msgDataDefaults
													CaoLiao.models.Messages.createWithTypeRoomIdMessageAndUser 'message_pinned', room._id, '', @getRocketUser(message.user), msgObj
												else
													#TODO: make this better
													@logger.debug('Pinned item with no attachment, needs work.');
													#CaoLiao.models.Messages.createWithTypeRoomIdMessageAndUser 'message_pinned', room._id, '', @getRocketUser(message.user), msgDataDefaults
											...
											if user?
												...
												CaoLiao.sendMessage @getRocketUser(message.user), msgObj, room, upsert: true
									@addCountCompleted 1
	...
	getRocketUser: (slackId) =>
		for user in @users.users when user.id is slackId
			return CaoLiao.models.Users.findOneById user.rocketId, { fields: { username: 1 }}

	convertSlackMessageToCaoLiao: (message) =>
		if message?
