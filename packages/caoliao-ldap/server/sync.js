...
slug = function slug(text) {
	if (CaoLiao.settings.get('UTF8_Names_Slugify') !== true) {
		return text;
	}
	text = slugify(text, '.');
...
getLdapUsername = function getLdapUsername(ldapUser) {
	let usernameField = CaoLiao.settings.get('LDAP_Username_Field');
...
getLdapUserUniqueID = function getLdapUserUniqueID(ldapUser) {
	let Unique_Identifier_Field = CaoLiao.settings.get('LDAP_Unique_Identifier_Field');
	...
	let LDAP_Domain_Search_User_ID = CaoLiao.settings.get('LDAP_Domain_Search_User_ID');
...
getDataToSyncUserData = function getDataToSyncUserData(ldapUser, user) {
	const syncUserData = CaoLiao.settings.get('LDAP_Sync_User_Data');
	const syncUserDataFieldMap = CaoLiao.settings.get('LDAP_Sync_User_Data_FieldMap').trim();
...
syncUserData = function syncUserData(user, ldapUser) {
	...
	if (user && user._id && username !== user.username) {
		...
		CaoLiao._setUsername(user._id, username);
	}

	if (user && user._id && CaoLiao.settings.get('LDAP_Sync_User_Avatar') === true) {
		const avatar = ldapUser.raw.thumbnailPhoto || ldapUser.raw.jpegPhoto;
		if (avatar) {
			logger.info('Syncing user avatar');
			const rs = CaoLiaoFile.bufferToStream(avatar);
			CaoLiaoFileAvatarInstance.deleteFile(encodeURIComponent(`${user.username}.jpg`));
			const ws = CaoLiaoFileAvatarInstance.createWriteStream(encodeURIComponent(`${user.username}.jpg`), 'image/jpeg');
			ws.on('end', Meteor.bindEnvironment(function() {
				Meteor.setTimeout(function() {
					CaoLiao.models.Users.setAvatarOrigin(user._id, 'ldap');
...
sync = function sync() {
	if (CaoLiao.settings.get('LDAP_Enable') !== true) {
		return;
	}
	...
	try {
		...
		const users = CaoLiao.models.Users.findLDAPUsers();
...
CaoLiao.settings.get('LDAP_Sync_User_Data', function(key, value) {
