...
Template.admin.helpers
	...
	appLanguage: (key) ->
		if !key
			return !CaoLiao.settings.get('Language')
		selected = (CaoLiao.settings.get('Language'))?.split('-').shift().toLowerCase() is key
		return selected
	...
	selectedOption: (_id, val) ->
		return CaoLiao.settings.get(_id) is val
...
Template.admin.onRendered ->
	...
	Meteor.setTimeout ->
		$('input.minicolors').minicolors({theme: 'caoliao'})
	, 1000
