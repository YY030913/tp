...
class IrcClient
	...
	onReceiveMessage: (source, target, content) ->
		...
		if target[0] == '#'
			room = CaoLiao.models.Rooms.findOneByName target.substring(1)
		else
		...
		CaoLiao.sendMessage source, message, room
	...
	onEndMemberList: (roomName) ->
		...
		room = CaoLiao.models.Rooms.findOneByNameAndType roomName, 'c'
		unless room
			return
		...
		CaoLiao.models.Rooms.removeUsernamesById room._id, removeMembers
		CaoLiao.models.Rooms.addUsernamesById room._id, appendMembers
	...
	initRoomList: ->
		roomsCursor = CaoLiao.models.Rooms.findByTypeContainigUsername 'c', @user.username,
			fields:
				name: 1
				t: 1
	...
	createDirectRoomWhenNotExist: (source, target) ->
		...
		CaoLiao.models.Subscriptions.upsert
			rid: rid
...
class IrcSender
	constructor: (message) ->
		...
		room = CaoLiao.models.Rooms.findOneById message.rid, { fields: { name: 1, usernames: 1, t: 1 } }
...
CaoLiao.callbacks.add 'beforeValidateLogin', IrcLoginer, CaoLiao.callbacks.priority.LOW
CaoLiao.callbacks.add 'beforeSaveMessage', IrcSender, CaoLiao.callbacks.priority.LOW
CaoLiao.callbacks.add 'beforeJoinRoom', IrcRoomJoiner, CaoLiao.callbacks.priority.LOW
CaoLiao.callbacks.add 'beforeCreateChannel', IrcRoomJoiner, CaoLiao.callbacks.priority.LOW
CaoLiao.callbacks.add 'beforeLeaveRoom', IrcRoomLeaver, CaoLiao.callbacks.priority.LOW
CaoLiao.callbacks.add 'afterLogoutCleanUp', IrcLogoutCleanUper, CaoLiao.callbacks.priority.LOW
