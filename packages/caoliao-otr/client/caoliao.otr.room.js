...
CaoLiao.OTR.Room = class {
	...
	handshake(refresh) {
		...
		this.generateKeyPair().then(() => {
			CaoLiao.Notifications.notifyUser(this.peerId, 'otr', 'handshake', { roomId: this.roomId, userId: this.userId, publicKey: EJSON.stringify(this.exportedPublicKey), refresh: refresh });
		});
	}

	acknowledge() {
		CaoLiao.Notifications.notifyUser(this.peerId, 'otr', 'acknowledge', { roomId: this.roomId, userId: this.userId, publicKey: EJSON.stringify(this.exportedPublicKey) });
	}

	deny() {
		this.reset();
		CaoLiao.Notifications.notifyUser(this.peerId, 'otr', 'deny', { roomId: this.roomId, userId: this.userId });
	}

	end() {
		this.reset();
		CaoLiao.Notifications.notifyUser(this.peerId, 'otr', 'end', { roomId: this.roomId, userId: this.userId });
	}
	...
	generateKeyPair() {
		...
		return CaoLiao.OTR.crypto.generateKey({
			name: 'ECDH',
			namedCurve: 'P-256'
		}, false, ['deriveKey', 'deriveBits']).then((keyPair) => {
			this.keyPair = keyPair;
			return CaoLiao.OTR.crypto.exportKey('jwk', keyPair.publicKey);
		})
	...
	importPublicKey(publicKey) {
		return CaoLiao.OTR.crypto.importKey('jwk', EJSON.parse(publicKey), {
			name: 'ECDH',
			namedCurve: 'P-256'
		}, false, []).then((peerPublicKey) => {
			return CaoLiao.OTR.crypto.deriveBits({
				name: 'ECDH',
				namedCurve: 'P-256',
				public: peerPublicKey
			}, this.keyPair.privateKey, 256);
		}).then((bits) => {
			return CaoLiao.OTR.crypto.digest({
				name: 'SHA-256'
			}, bits);
		}).then((hashedBits) => {
			// We truncate the hash to 128 bits.
			var sessionKeyData = new Uint8Array(hashedBits).slice(0, 16);
			return CaoLiao.OTR.crypto.importKey('raw', sessionKeyData, {
				name: 'AES-GCM'
	...
	encryptText(data) {
		...
		return CaoLiao.OTR.crypto.encrypt({
			name: 'AES-GCM',
			iv: iv
	...
	decrypt(message) {
		...
		return CaoLiao.OTR.crypto.decrypt({
			name: 'AES-GCM',
			iv: iv
