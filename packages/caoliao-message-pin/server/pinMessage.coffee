Meteor.methods
	pinMessage: (message) ->
		...
		if CaoLiao.settings.get 'Message_KeepHistory'
			CaoLiao.models.Messages.cloneAndSaveAsHistoryById message._id
		...
		CaoLiao.models.Messages.setPinnedByIdAndUserId message._id, message.pinnedBy, message.pinned

		CaoLiao.models.Messages.createWithTypeRoomIdMessageAndUser 'message_pinned', message.rid, '', me,
			attachments: [
				"text" : message.msg
	...
	unpinMessage: (message) ->
		...
		# Meteor.defer ->
		# 	CaoLiao.callbacks.run 'afterSaveMessage', CaoLiao.models.Messages.findOneById(message.id)
