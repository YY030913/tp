...
Tracker.autorun ->
	...
	if connected is true and connectionWasOnline is false and RoomManager.openedRooms?
		...
		for key, value of RoomManager.openedRooms
			if value.rid?
				console.log "autorun loadMissedMessages",value.rid
	...
	close = (typeName) ->
		if openedRooms[typeName]
			...
			if openedRooms[typeName].rid?
				...
				CaoLiao.Notifications.unRoom openedRooms[typeName].rid, 'deleteMessage', onDeleteMessageStream
			...
			do (typeName, record) ->
				...
				if ready is true
					...
					room = Tracker.nonreactive =>
						return CaoLiao.roomTypes.findRoom(type, name, user)
					...
					if not room?
						...
						if openedRooms[typeName].streamActive isnt true
							...
							msgStream.on openedRooms[typeName].rid, (msg) ->
								#console.log "msgStream.on openedRooms", openedRooms, msg
								CaoLiao.promises.run('onClientMessageReceived', msg).then (msg) ->
									...
									if RoomHistoryManager.hasMoreNext(openedRooms[typeName].rid) is false
										...
										CaoLiao.callbacks.run 'streamMessage', msg

							CaoLiao.Notifications.onRoom openedRooms[typeName].rid, 'deleteMessage', onDeleteMessageStream
