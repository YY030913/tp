class @ChatMessages
	init: (node) ->
		this.editing = {}
		this.records  = {}
		this.messageMaxSize = CaoLiao.settings.get('Message_MaxAllowedSize')
	...
	send: (rid, input, done = ->) ->
		if _.trim(input.value) isnt ''
			...
			CaoLiao.promises.run('onClientBeforeSendMessage', msgObject).then (msgObject) =>
				...
				if msg[0] is '/'
					...
					if match? and CaoLiao.slashCommands.commands[match[1]]
						command = match[1]
						param = match[2]
	...
	confirmDeleteMsg: (message, done = ->) ->
		return if CaoLiao.MessageTypes.isSystemMessage(message)
		swal {
