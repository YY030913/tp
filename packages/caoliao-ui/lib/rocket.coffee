CaoLiao.Login = (->
		...
		onBlur = (input) ->
			...
			if $input.length
				if input.value == ""
					$input.parents(".input-field").removeClass "active"
		check = (form) ->
			$form = $(form)
			if $form.length
				...
				inputs.each ->
					if @.value != ""
						console.log @.value
						$(@).parents(".input-field").addClass "active"
		check: check
		onClick: onClick
		onBlur: onBlur
	)()
...
CaoLiao.animeBack = (el, callback, type) ->
		el = $(el)
		if not el.length > 0
...
CaoLiao.preLoadImgs = (urls, callback) ->
		L_ = (x) ->
			if x.width > 0
