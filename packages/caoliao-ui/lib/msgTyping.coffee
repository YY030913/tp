@MsgTyping = do ->
	...
	addStream = (room) ->
		if _.isEmpty usersTyping[room]?.users
			...
			CaoLiao.Notifications.onRoom room, 'typing', (username, typing) ->
	...
	start = (room) ->
		...
		renew = false
		selfTyping.set true
		CaoLiao.Notifications.notifyRoom room, 'typing', Meteor.user()?.username, true
	...
	stop = (room) ->
		...
		if timeouts?[room]?
			clearTimeout(timeouts[room])
			timeouts[room] = null
		CaoLiao.Notifications.notifyRoom room, 'typing', Meteor.user()?.username, false
