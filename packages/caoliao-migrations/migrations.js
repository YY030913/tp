...
Migrations = {
	...
	options: {
		...
		collectionName: "migrations"
			// collectionName: "caoliao_migrations"
	},
...
Migrations.migrateTo = function(command) {
	...
	if (!migrated) {
		...
		console.log(makeABox([
			...
			"",
			"This CaoLiao version: " + CaoLiao.Info.version,
			"Database locked at version: " + control.version,
			"Database target version: " + (version === 'latest' ? _.last(this._list).version : version),
			"",
			"Commit: " + CaoLiao.Info.commit.hash,
			"Date: " + CaoLiao.Info.commit.date,
			"Branch: " + CaoLiao.Info.commit.branch,
			"Tag: " + CaoLiao.Info.commit.tag
		]));
		process.exit(1);
	}
...
Migrations._migrateTo = function(version, rerun) {
	...
	function lock() {
		...
		const build = CaoLiao.Info ? CaoLiao.Info.build.date : date;
