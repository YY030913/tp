...
(function (win, doc) {
    // 压缩代码相关
    /* compressor */
    objectName = win.alogObjectName || 'alog';
    oldObject = win[objectName];
    if (oldObject && oldObject.defined) { // 避免重复加载
        return;
    }
    /*<ie>*/
    ie = win.attachEvent && !window.opera;
    /**
     * 是否IE
     */
    /**
    ...
    function loadModules(moduleName) {
        modulesConfig = defaultTracker.get('alias') || {};
        scriptUrl = modulesConfig[moduleName] || (moduleName + '.js');
        ...
        lastElement = doc.getElementsByTagName(scriptTag)[0];
    ...
    function clearDeps(module) {
        ...
        if (requires) {
            for (i = 0; i < requires.length; i++) {
                ...
                deps = modules[moduleName] = (modules[moduleName] || {});
    ...
    function clearWaiting(module) {
        for (moduleName in module.waiting) {
    ...
    function un(element, eventName, callback) {
        ...
        try {
            if (element === entry) {
                listener = alogListeners[eventName];
                if (!listener) {
                    return;
                }
                i = listener.length;
    ...
    function command() {
        ...
        if (this.created || /^(on|un|set|get|create)$/.test(method)) {
            methodFunc = Tracker.prototype[method];
            params = [];
            for (i = 1, len = args.length; i < len; i++) {
                params.push(args[i]);
            }
    ...
    if (oldObject) {
        // 处理临时alog对象
        items = [].concat(oldObject.p || [], oldObject.q || []);
