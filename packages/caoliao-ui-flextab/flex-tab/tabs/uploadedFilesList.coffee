...
Template.uploadedFilesList.helpers
	...
	canDelete: ->
		return CaoLiao.authz.hasAtLeastOnePermission('delete-message', @rid) or CaoLiao.settings.get('Message_AllowDeleting') and @userId is Meteor.userId()
...
Template.uploadedFilesList.events
	...
	'click .icon-trash': (e, t) ->
		...
		swal {
			...
			CaoLiao.models.Uploads.remove self._id, () ->
				if msg
