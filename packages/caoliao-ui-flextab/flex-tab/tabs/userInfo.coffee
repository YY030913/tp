Template.userInfo.helpers
	...
	canDirectMessage: (username) ->
		return CaoLiao.authz.hasAllPermission('create-d') and Meteor.user()?.username isnt username
	...
	canRemoveUser: ->
		return CaoLiao.authz.hasAllPermission('remove-user', Session.get('openedRoom'))

	canMuteUser: ->
		return CaoLiao.authz.hasAllPermission('mute-user', Session.get('openedRoom'))
	...
	canSetModerator: ->
		return CaoLiao.authz.hasAllPermission('set-moderator', Session.get('openedRoom'))

	isModerator: ->
		return !!RoomRoles.findOne({ rid: Session.get('openedRoom'), "u._id": Template.instance().user.get()?._id, roles: 'moderator' })

	canSetOwner: ->
		return CaoLiao.authz.hasAllPermission('set-owner', Session.get('openedRoom'))
	...
	hasAdminRole: ->
		return CaoLiao.authz.hasRole(Template.instance().user.get()?._id, 'admin')
...
Template.userInfo.events
	...
	'click .remove-user': (e, instance) ->
		...
		if CaoLiao.authz.hasAllPermission('remove-user', rid)
			swal {
	...
	'click .mute-user': (e, instance) ->
		...
		rid = Session.get('openedRoom')
		room = ChatRoom.findOne rid
		if CaoLiao.authz.hasAllPermission('mute-user', rid)
			swal {
				title: t('Are_you_sure')
