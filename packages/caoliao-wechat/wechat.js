...
// 验证
Weixin.checkSignature = function(req) {    		
...
// ------------------ 监听 ------------------------
// 监听文本消息
Weixin.textMsg = function(callback) {
...
// 监听图片消息
Weixin.imageMsg = function(callback) {
...
// 监听语音消息
Weixin.voiceMsg = function(callback) {
...
// 监听地理位置消息
Weixin.locationMsg = function(callback) {
...
// 监听链接消息
Weixin.urlMsg = function(callback) {
...
// 监听事件
Weixin.eventMsg = function(callback) {
 ...
 * MsgId	 消息id，64位整型
 */
Weixin.parseLocationMsg = function(data) {
	msg = {
		"toUserName" : this.data.ToUserName[0],
		"fromUserName" : this.data.FromUserName[0],
		"createTime" : this.data.CreateTime[0],
...
// --------------------- 消息返回 -------------------------
// 返回文字信息
Weixin.sendTextMsg = function(msg) {
	time = Math.round(new Date().getTime() / 1000);
	
	funcFlag = msg.funcFlag ? msg.funcFlag : this.funcFlag;
...
// 返回音乐信息
Weixin.sendMusicMsg = function(msg) {
	time = Math.round(new Date().getTime() / 1000);
...
// 返回图文信息
Weixin.sendNewsMsg = function(msg) {
	time = Math.round(new Date().getTime() / 1000);
	
	// 
	articlesStr = "";	
	for (i = 0; i < msg.articles.length; i++) 
	{
		articlesStr += "<item>" + 
