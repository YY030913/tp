Template.accountProfile.helpers
	allowDeleteOwnAccount: ->
		return CaoLiao.settings.get('Accounts_AllowDeleteOwnAccount')
	...
	allowUsernameChange: ->
		return CaoLiao.settings.get("Accounts_AllowUsernameChange") and CaoLiao.settings.get("LDAP_Enable") isnt true

	allowEmailChange: ->
		return CaoLiao.settings.get("Accounts_AllowEmailChange")

	usernameChangeDisabled: ->
		return t('Username_Change_Disabled')

	allowPasswordChange: ->
		return CaoLiao.settings.get("Accounts_AllowPasswordChange")
...
Template.accountProfile.onCreated ->
	...
	@changePassword = (newPassword, callback) ->
		...
		if not newPassword
			...
			if !CaoLiao.settings.get("Accounts_AllowPasswordChange")
				toastr.remove();
...
Template.accountProfile.onRendered ->
	Tracker.afterFlush ->
		...
		FlowRouter.go("home") if !CaoLiao.settings.get("Accounts_AllowUserProfileChange")
		###
