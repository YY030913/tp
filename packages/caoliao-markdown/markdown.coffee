...
class Markdown
	constructor: (message) ->
		...
		schemes = CaoLiao.settings.get('Markdown_SupportSchemesForLink').split(',').join('|')
		...
		if CaoLiao.settings.get('Markdown_Headers')
			# Support # Text for h1
...
CaoLiao.callbacks.add 'renderMessage', Markdown, CaoLiao.callbacks.priority.HIGH
CaoLiao.Markdown = Markdown

if Meteor.isClient
	Blaze.registerHelper 'CaoLiaoMarkdown', (text) ->
		return CaoLiao.Markdown text
