...
@CaoLiaoAssetsInstance = new CaoLiaoFile.GridFS
	name: 'assets'
...
CaoLiao.Assets = new class
	mime: mime
	assets: assets

	setAsset: (binaryContent, contentType, asset) ->
		if not assets[asset]?
			throw new Meteor.Error "error-invalid-asset", 'Invalid asset', { function: 'CaoLiao.Assets.setAsset' }

		extension = mime.extension(contentType)
		if extension not in assets[asset].constraints.extensions
			throw new Meteor.Error contentType, 'Invalid file type: ' + contentType, { function: 'CaoLiao.Assets.setAsset', errorTitle: 'error-invalid-file-type' }
		...
		rs = CaoLiaoFile.bufferToStream file
		CaoLiaoAssetsInstance.deleteFile asset
		ws = CaoLiaoAssetsInstance.createWriteStream asset, contentType
		ws.on 'end', Meteor.bindEnvironment ->
			Meteor.setTimeout ->
				CaoLiao.settings.updateById "Assets_#{asset}", {
	...
	unsetAsset: (asset) ->
		if not assets[asset]?
			throw new Meteor.Error "error-invalid-asset", 'Invalid asset', { function: 'CaoLiao.Assets.unsetAsset' }

		CaoLiaoAssetsInstance.deleteFile asset

		CaoLiao.settings.updateById "Assets_#{asset}", {defaultUrl: assets[asset].defaultUrl}
		return
...
CaoLiao.settings.addGroup 'Assets'
for key, value of assets
	do (key, value) ->
		CaoLiao.settings.add "Assets_#{key}", {defaultUrl: value.defaultUrl}, { type: 'asset', group: 'Assets', fileConstraints: value.constraints, i18nLabel: value.label, asset: key, public: true }

Meteor.startup ->
	forEachAsset = (key, value) ->
		CaoLiao.settings.get "Assets_#{key}", (settingKey, settingValue) ->
			if not settingValue?.url?
				value.cache = undefined
				return

			file = CaoLiaoAssetsInstance.getFileWithReadStream key
			if not file
				value.cache = undefined
				return
...
Meteor.methods
	refreshClients: ->
		...
		hasPermission = CaoLiao.authz.hasPermission Meteor.userId(), 'manage-assets'
	...
	unsetAsset: (asset) ->
		...
		CaoLiao.Assets.unsetAsset asset
	...
	setAsset: (binaryContent, contentType, asset) ->
		...
		CaoLiao.Assets.setAsset binaryContent, contentType, asset
		return
