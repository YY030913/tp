Template.listChannelsFlex.helpers
	...
	member: ->
		return !!CaoLiao.models.Subscriptions.findOne({ name: @name, open: true })
	hidden: ->
		return !!CaoLiao.models.Subscriptions.findOne({ name: @name, open: false })
		...
		if @show.get() is 'joined'
			...
			if _.trim(@sortSubscriptions.get())
				switch @sortSubscriptions.get()
					...
					when 'ls'
						options.sort = { ls: -1 }
			@channelsList.set CaoLiao.models.Subscriptions.find({
