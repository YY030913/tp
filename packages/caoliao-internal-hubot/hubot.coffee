...
class CaoLiaoAdapter extends Hubot.Adapter
	...
	send: (envelope, strings...) ->
		console.log 'caoliaoADAPTER -> send'.blue
		...
		sendHelper @robot, envelope, strings, (string) =>
			...
			CaoLiao.sendMessage InternalHubot.user, { msg: string }, { _id: envelope.room }
	...
	emote: (envelope, strings...) ->
		console.log 'caoliaoADAPTER -> emote'.blue
		sendHelper @robot, envelope, strings, (string) =>
	...
	priv: (envelope, strings...) ->
		console.log 'caoliaoADAPTER -> priv'.blue
		sendHelper @robot, envelope, strings, (string) ->
	...
	reply: (envelope, strings...) ->
		console.log 'caoliaoADAPTER -> reply'.blue
	...
	topic: (envelope, strings...) ->
		console.log 'caoliaoADAPTER -> topic'.blue
	...
	play: (envelope, strings...) ->
		console.log 'caoliaoADAPTER -> play'.blue

	# Public: Raw method for invoking the bot to run. Extend this.
	#
	# Returns nothing.
	run: ->
		console.log 'caoliaoADAPTER -> run'.blue
	...
	# Returns nothing.
	close: ->
		console.log 'caoliaoADAPTER -> close'.blue

class InternalHubotReceiver
	constructor: (message) ->
		#console.log message
		if message.u.username isnt InternalHubot.name
			room = CaoLiao.models.Rooms.findOneById message.rid
	...
	constructor: (robot) ->
		...
		for modulePath in modulesToLoad
			try
				...
				robot.parseHelp __meteor_bootstrap__.serverDir+'/npm/caoliao_internal-hubot/node_modules/'+modulePath
				console.log "Loaded #{modulePath}".green
			catch e
				console.log "can't load #{modulePath}".red
				console.log e

		scriptsToLoad = CaoLiao.settings.get('InternalHubot_ScriptsToLoad').split(',') or []
		...
		for scriptFile in scriptsToLoad
			try
				...
				# robot.loadFile __meteor_bootstrap__.serverDir+'/npm/caoliao_internal-hubot/node_modules/hubot-scripts/src/scripts', scriptFile
				robot.parseHelp __meteor_bootstrap__.serverDir+'/npm/caoliao_internal-hubot/node_modules/hubot-scripts/src/scripts/'+scriptFile
		...
		# npm/caoliao_internal-hubot/node_modules
		# packages/caoliao_internal-hubot.js
	...
	while strings.length > 0
		...
		if typeof(string) == 'function'
			...
			try
				...
				robot.logger.error "CaoLiao send error: #{err}"

InternalHubot = {}

init = =>
	InternalHubot = new Robot null, null, false, CaoLiao.settings.get 'InternalHubot_Username'
	InternalHubot.alias = 'bot'
	InternalHubot.adapter = new CaoLiaoAdapter InternalHubot
	...
	if CaoLiao.settings.get 'InternalHubot_Enabled'
		CaoLiao.callbacks.add 'afterSaveMessage', InternalHubotReceiver, CaoLiao.callbacks.priority.LOW, 'InternalHubot'
	else
		CaoLiao.callbacks.remove 'afterSaveMessage', 'InternalHubot'
...
Meteor.startup ->
	init()
	CaoLiao.models.Settings.findByIds([ 'InternalHubot_Username', 'InternalHubot_Enabled', 'InternalHubot_ScriptsToLoad']).observe
		changed: ->
			init()
