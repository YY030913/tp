...
Template.integrationsOutgoing.helpers
	...
	data: ->
		...
		if params?.id?
			data = null
			if CaoLiao.authz.hasAllPermission 'manage-integrations'
				data = ChatIntegrations.findOne({_id: params.id})
			else if CaoLiao.authz.hasAllPermission 'manage-own-integrations'
	...
	example: ->
		...
		return {} =
			...
			attachments: [{
				title: "CaoLiao"
				title_link: "https://caoliao"
				text: "CaoLiao, the best open source chat"
				image_url: "https://caoliao/images/mockup.png"
				color: "#764FA5"
			}]
			ts: new Date
			u:
