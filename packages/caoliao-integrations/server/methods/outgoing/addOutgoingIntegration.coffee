Meteor.methods
	addOutgoingIntegration: (integration) ->
		...
		if (not CaoLiao.authz.hasPermission @userId, 'manage-integrations') and
		not (CaoLiao.authz.hasPermission @userId, 'manage-own-integrations') and
		not (CaoLiao.authz.hasPermission @userId, 'manage-integrations', 'bot') and
		not (CaoLiao.authz.hasPermission @userId, 'manage-own-integrations', 'bot')
		...
		integration._createdBy = CaoLiao.models.Users.findOne @userId, {fields: {username: 1}}

		integration._id = CaoLiao.models.Integrations.insert integration
