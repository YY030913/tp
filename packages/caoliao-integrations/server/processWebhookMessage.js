this.processWebhookMessage = function(messageObj, user, defaultValues) {
	...
	for (channel of channels) {
		...
		switch (channelType) {
			case '#':
				room = CaoLiao.models.Rooms.findOne({
					$or: [
						{
							_id: channel
						}, {
				...
				break;
			case '@':
				roomUser = CaoLiao.models.Users.findOne({
					$or: [
						{
							_id: channel
						}, {
				...
				if (!room) {
					Meteor.runAsUser(user._id, function() {
						...
						room = CaoLiao.models.Rooms.findOne(rid);
					});
				}
				break;
			default:
		...
		var messageReturn = CaoLiao.sendMessage(user, message, room, {});
