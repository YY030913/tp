...
CaoLiao.models.Integrations.find({type: 'webhook-outgoing'}).observe
	added: (record) ->
...
ExecuteTriggerUrl = (url, trigger, message, room, tries=0) ->
	...
	sendMessage = (message) ->
		user = CaoLiao.models.Users.findOneByUsername(trigger.username)
	...
	opts =
		...
		auth: undefined
		npmRequestOptions:
			rejectUnauthorized: !CaoLiao.settings.get 'Allow_Invalid_SelfSigned_Certs'
	...
	HTTP.call opts.method, opts.url, opts, (error, result) ->
		...
		if not result? or result.statusCode not in [200, 201, 202]
			...
			if result.statusCode is 410
				CaoLiao.models.Integrations.remove _id: trigger._id
				return
...
CaoLiao.callbacks.add 'afterSaveMessage', ExecuteTriggers, CaoLiao.callbacks.priority.LOW
