...
getIntegrationScript = (integration) ->
	...
	try
		...
		throw CaoLiao.API.v1.failure 'error-evaluating-script'

	if not sandbox.Script?
		throw CaoLiao.API.v1.failure 'class-script-not-found'
...
Api = new Restivus
	...
	auth:
		user: ->
			...
			@integration = CaoLiao.models.Integrations.findOne
			...
			user = CaoLiao.models.Users.findOne
...
removeIntegration = (options, user) ->
	...
	integrationToRemove = CaoLiao.models.Integrations.findOne urls: options.target_url
	Meteor.runAsUser user._id, =>
...
executeIntegrationRest = ->
	...
	if @integration.scriptEnabled is true and @integration.scriptCompiled? and @integration.scriptCompiled.trim() isnt ''
		...
		try
			...
			if result?.error?
				return CaoLiao.API.v1.failure result.error
			...
			logger.incoming.error "[Stack:]"
			logger.incoming.error e.stack.replace(/^/gm, '  ')
			return CaoLiao.API.v1.failure 'error-running-script'

	if not @bodyParams?
		CaoLiao.API.v1.failure 'body-empty'
...
CaoLiao.API.v1.addRoute 'integrations.create', authRequired: true,
	post: ->
		return createIntegration @bodyParams, @user


CaoLiao.API.v1.addRoute 'integrations.remove', authRequired: true,
	post: ->
		return removeIntegration @bodyParams, @user
