...
getUrlContent = (urlObj, redirectCount = 5, callback) ->
	...
	ignoredHosts = CaoLiao.settings.get('API_EmbedIgnoredHosts').replace(/\s/g, '').split(',') or []
	if parsedUrl.hostname in ignoredHosts or ipRangeCheck(parsedUrl.hostname, ignoredHosts)
		return callback()

	data = CaoLiao.callbacks.run 'oembed:beforeGetUrlContent',
		urlObj: urlObj
		parsedUrl: parsedUrl
	...
	opts =
		url: url
		strictSSL: !CaoLiao.settings.get 'Allow_Invalid_SelfSigned_Certs'
		gzip: true
...
OEmbed.getUrlMeta = (url, withFragment) ->
	...
	data = CaoLiao.callbacks.run 'oembed:afterParseContent',
		meta: metas
		headers: headers
...
OEmbed.getUrlMetaWithCache = (url, withFragment) ->
	cache = CaoLiao.models.OEmbedCache.findOneById url
	if cache?
		return cache.data
	...
	if data?
		try
			CaoLiao.models.OEmbedCache.createWithIdAndData url, data
		catch e
...
OEmbed.RocketUrlParser = (message) ->
	if Array.isArray message.urls
		...
		if attachments.length
			CaoLiao.models.Messages.setMessageAttachments message._id, attachments

		if changed is true
			CaoLiao.models.Messages.setUrlsById message._id, message.urls

	return message

CaoLiao.settings.get 'API_Embed', (key, value) ->
	if value
		CaoLiao.callbacks.add 'afterSaveMessage', OEmbed.RocketUrlParser, CaoLiao.callbacks.priority.LOW, 'API_Embed'
	else
		CaoLiao.callbacks.remove 'afterSaveMessage', 'API_Embed'
