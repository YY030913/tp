## CaoLiao main library

This package contains the main libraries of CaoLiao.
...
```javascript
CaoLiao.settings.addGroup('Settings_Group', function() {
...
CaoLiao.roomTypes.add('l', 5, {
    ...
    condition: () => {
        return CaoLiao.authz.hasAllPermission('view-l-room');
    }
});
```
...
```javascript
CaoLiao.roomTypes.setPublish('l', (identifier) => {
    return CaoLiao.models.Rooms.findByTypeAndName('l', identifier, {
        fields: {
            name: 1,
            t: 1,
            cl: 1,
...
AccountBox.addItem({
    ...
    condition: () => {
        return CaoLiao.authz.hasAllPermission('view-livechat-manager');
    }
});
```
