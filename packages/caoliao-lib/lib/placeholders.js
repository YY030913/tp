CaoLiao.placeholders = {};

CaoLiao.placeholders.replace = function(str, data) {
	if (!str) {
		return '';
	}

	str = str.replace(/\[Site_Name\]/g, CaoLiao.settings.get('Site_Name') || '');
	str = str.replace(/\[Site_URL\]/g, CaoLiao.settings.get('Site_Url') || '');
