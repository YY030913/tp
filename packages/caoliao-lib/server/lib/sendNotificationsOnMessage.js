...
CaoLiao.callbacks.add('afterSaveMessage', function(message, room) {
	...
	var user = CaoLiao.models.Users.findOneById(message.u._id);
	...
	CaoLiao.models.Subscriptions.findNotificationPreferencesByRoom(room._id).forEach(function(subscription) {
	...
	userIdsToPushNotify = [];
	usersWithHighlights = [];
	highlights = CaoLiao.models.Users.findUsersByUsernamesWithHighlights(room.usernames, { fields: { '_id': 1, 'settings.preferences.highlights': 1 }}).fetch();
	...
	if (CaoLiao.settings.get('Push_show_message')) {
	...
	let push_username;
	let push_room;
	if (CaoLiao.settings.get('Push_show_username_room')) {
	...
	if ((room.t == null) || room.t === 'd') {
		...
		userOfMention = CaoLiao.models.Users.findOne({
			_id: userOfMentionId
		}, {
			fields: {
				username: 1,
		...
		if ((userOfMention != null) && canBeNotified(userOfMentionId, 'mobile')) {
			CaoLiao.Notifications.notifyUser(userOfMention._id, 'notification', {
				title: '@' + user.username,
				text: message.msg,
				payload: {
					rid: message.rid,
		...
		if (mentionIds.length > 0 || settings.alwaysNotifyDesktopUsers.length > 0) {
			...
			usersOfDesktopMentions = CaoLiao.models.Users.find({
				_id: {
		...
		if (mentionIds.length > 0 || settings.alwaysNotifyMobileUsers.length > 0) {
			...
			usersOfMobileMentions = CaoLiao.models.Users.find({
				_id: {
					$in: mobileMentionIds
				}
			}, {
		...
		if (userIdsToNotify.length > 0) {
			for (j = 0, len1 = userIdsToNotify.length; j < len1; j++) {
				...
				CaoLiao.Notifications.notifyUser(usersOfMentionId, 'notification', {
					title: title,
					text: message.msg,
					payload: {
						rid: message.rid,
