CaoLiao.callbacks.add('afterSaveMessage', function(message, room) {
	...
	if (room.t === 'd') {
		...
		emailSubject = TAPi18n.__('Offline_DM_Email', {
			site: CaoLiao.settings.get('Site_Name'),
	...
	// code duplicate of packages/caoliao-ui-message/message/message.coffee
	message.html = s.escapeHTML(message.msg);
	message = CaoLiao.callbacks.run('renderMessage', message);
	...
	var header = CaoLiao.placeholders.replace(CaoLiao.settings.get('Email_Header') || '');
	var footer = CaoLiao.placeholders.replace(CaoLiao.settings.get('Email_Footer') || '');
	message.html = header + message.html.replace(/\n/gm, '<br/>') + footer;

	CaoLiao.models.Subscriptions.findWithSendEmailByRoomId(room._id).forEach((sub) => {
	...
	if (userIdsToSendEmail.length > 0) {
		var usersOfMention = CaoLiao.models.Users.getUsersToSendOfflineEmail(userIdsToSendEmail).fetch();
