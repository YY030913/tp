Meteor.methods({
	getUserRoles() {
		...
		const roles = CaoLiao.models.Roles.find({ scope: 'Users', description: { $exists: 1, $ne: '' } }).fetch();
		...
		let users = CaoLiao.models.Users.findUsersInRoles(roleIds, null, options).fetch();
