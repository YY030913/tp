(function(w) {
	...
	var init = function(url) {
		...
		chatWidget.className = 'caoliao-widget';
		chatWidget.innerHTML = '<div class="caoliao-container" style="width:100%;height:100%">' +
								'<iframe id="caoliao-iframe" src="' + url + '" style="width:100%;height:100%;border:none;background-color:transparent" allowTransparency="true"></iframe> '+
								'</div><div class="caoliao-overlay"></div>';
		...
		widget = document.querySelector('.caoliao-widget');
		iframe = document.getElementById('caoliao-iframe');

		w.addEventListener('message', function(msg) {
			if (typeof msg.data === 'object' && msg.data.src !== undefined && msg.data.src === 'caoliao') {
	...
	if (typeof w.CaoLiao.url !== 'undefined') {
		init(w.CaoLiao.url);
	}

	var queue = w.CaoLiao._;

	w.CaoLiao = w.CaoLiao._.push = function(c) {
		c.call(w.CaoLiao.livechat);
	};
