 ...
 */
class LivechatDepartment extends CaoLiao.models._Base {
	constructor() {
		super();
	...
	createOrUpdateDepartment(_id, enabled, name, description, agents, extraData) {
		...
		var savedAgents = _.pluck(CaoLiao.models.LivechatDepartmentAgents.findByDepartmentId(_id).fetch(), 'agentId');
		var agentsToSave = _.pluck(agents, 'agentId');

		// remove other agents
		_.difference(savedAgents, agentsToSave).forEach((agentId) => {
			CaoLiao.models.LivechatDepartmentAgents.removeByDepartmentIdAndAgentId(_id, agentId);
		});

		agents.forEach((agent) => {
			CaoLiao.models.LivechatDepartmentAgents.saveAgent({
...
CaoLiao.models.LivechatDepartment = new LivechatDepartment();
