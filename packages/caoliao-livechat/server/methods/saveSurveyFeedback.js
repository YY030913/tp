...
Meteor.methods({
	'livechat:saveSurveyFeedback'(visitorToken, visitorRoom, formData) {
		...
		const visitor = CaoLiao.models.Users.getVisitorByToken(visitorToken);
		const room = CaoLiao.models.Rooms.findOneById(visitorRoom);
		...
		if (visitor !== undefined && room !== undefined && room.v !== undefined && visitor.profile !== undefined && room.v.token === visitor.profile.token) {
			...
			if (!_.isEmpty(updateData)) {
				return CaoLiao.models.Rooms.updateSurveyFeedbackById(room._id, updateData);
			}
		}
	}
});
