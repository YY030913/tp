...
Api.addRoute('sms-incoming/:service', {
	post() {
		const SMSService = CaoLiao.SMS.getService(this.urlParams.service);

		const sms = SMSService.parse(this.bodyParams);

		var visitor = CaoLiao.models.Users.findOneVisitorByPhone(sms.from);
		...
		if (visitor) {
			const rooms = CaoLiao.models.Rooms.findOpenByVisitorToken(visitor.profile.token).fetch();
			...
			let userId = CaoLiao.Livechat.registerGuest({
			...
			visitor = CaoLiao.models.Users.findOneById(userId);
		}
		...
		const message = SMSService.response.call(this, CaoLiao.Livechat.sendMessage(sendMessage));
