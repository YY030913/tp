CaoLiao.callbacks.add('afterSaveMessage', function(message, room) {
	...
	console.log("sendMessageBySms")

	if (!CaoLiao.SMS.enabled) {
		return message;
	}
	...
	const SMSService = CaoLiao.SMS.getService(CaoLiao.settings.get('SMS_Service'));

	if (!SMSService) {
		return message;
	}

	const visitor = CaoLiao.models.Users.getVisitorByToken(room.v.token);
