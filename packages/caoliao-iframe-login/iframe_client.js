...
class IframeLogin {
	constructor() {
		...
		Tracker.autorun((c) => {
			this.enabled = CaoLiao.settings.get('Accounts_iframe_enabled');
			this.reactiveEnabled.set(this.enabled);

			this.iframeUrl = CaoLiao.settings.get('Accounts_iframe_url');
			this.apiUrl = CaoLiao.settings.get('Accounts_Iframe_api_url');
			this.apiMethod = CaoLiao.settings.get('Accounts_Iframe_api_method');
...
CaoLiao.iframeLogin = new IframeLogin();
...
window.addEventListener('message', (e) => {
	...
	switch (e.data.event) {
		case 'try-iframe-login':
			CaoLiao.iframeLogin.tryLogin((error) => {
				if (error) {
		...
		case 'login-with-token':
			CaoLiao.iframeLogin.loginWithToken(e.data.token, (error) => {
				if (error) {
					e.source.postMessage({
						event: 'login-error',
						response: error.message
