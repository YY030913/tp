...
Template.loginForm.helpers
	...
	namePlaceholder: ->
		return if CaoLiao.settings.get 'Accounts_RequireNameForSignUp' then t('Name') else t('Name_optional')

	showFormLogin: ->
		return true;#CaoLiao.settings.get('Accounts_ShowFormLogin') and Template.instance().validSecretURL?.get()
	...
	loginTerms: ->
		return CaoLiao.settings.get 'Layout_Login_Terms'

	registrationAllowed: ->
		return CaoLiao.settings.get('Accounts_RegistrationForm') is 'Public' or Template.instance().validSecretURL?.get()

	linkReplacementText: ->
		return CaoLiao.settings.get('Accounts_RegistrationForm_LinkReplacementText')

	passwordresetAllowed: ->
		return CaoLiao.settings.get 'Accounts_PasswordReset'

	emailOrUsernamePlaceholder: ->
		return CaoLiao.settings.get('Accounts_EmailOrUsernamePlaceholder') or t("Email_or_username")

	passwordPlaceholder: ->
		return CaoLiao.settings.get('Accounts_PasswordPlaceholder') or t("Password")
...
Template.loginForm.events
		...
		if formData
			...
			if instance.state.get() is 'register'
				...
				Meteor.call 'registerUser', formData, (error, result) ->
					...
					Meteor.loginWithPassword s.trim(formData.email), formData.pp, (error) ->
						...
						#ran = ranaly.createClient();
						#rUsers = new ran.Amount('Users');
						#rUsers.incr();

			else
				loginMethod = 'loginWithPassword'
				if CaoLiao.settings.get('LDAP_Enable')
					loginMethod = 'loginWithLDAP'

				Meteor[loginMethod] s.trim(formData.useraccount), formData.pp, (error) ->
					CaoLiao.Button.reset(button)
					if error?
	...
	'click .one-passsword': ->
		...
		succesCallback = (credentials) ->
			$('input[name=useraccount]').val(credentials.username)
			$('input[name=pp]').val(credentials.password)
...
Template.loginForm.onCreated ->
	...
	@validate = ->
		...
		if instance.state.get() isnt 'forgot-password'
			unless formObj['pp']
				validationObj['pp'] = t('Invalid_pass')

		if instance.state.get() is 'register'
			if CaoLiao.settings.get('Accounts_RequireNameForSignUp') and not formObj['name']
				validationObj['name'] = t('Invalid_name')
			if formObj['confirm-pass'] isnt formObj['pp']
