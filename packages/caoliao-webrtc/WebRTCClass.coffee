...
class WebRTCTransportClass
	...
	constructor: (@webrtcInstance) ->
		...
		CaoLiao.Notifications.onRoom @webrtcInstance.room, 'webrtc', (type, data) =>
			switch type
				when 'status'
	...
	startCall: (data) ->
		...
		CaoLiao.Notifications.notifyUsersOfRoom @webrtcInstance.room, 'webrtc', 'call',
	...
	joinCall: (data) ->
		if data.monitor is true
			CaoLiao.Notifications.notifyUser data.to, 'webrtc', 'join',
				from: @webrtcInstance.selfId
				room: @webrtcInstance.room
				media: data.media
				monitor: data.monitor
		else
			CaoLiao.Notifications.notifyUsersOfRoom @webrtcInstance.room, 'webrtc', 'join',
				from: @webrtcInstance.selfId
				room: @webrtcInstance.room
				media: data.media
				monitor: data.monitor
	...
	sendCandidate: (data) ->
		...
		CaoLiao.Notifications.notifyUser data.to, 'webrtc', 'candidate', data
	...
	sendDescription: (data) ->
		...
		CaoLiao.Notifications.notifyUser data.to, 'webrtc', 'description', data

	sendStatus: (data) ->

		data.from = @webrtcInstance.selfId
		CaoLiao.Notifications.notifyRoom @webrtcInstance.room, 'webrtc', 'status', data
...
class WebRTCClass
	...
	constructor: (@selfId, @room) ->
		...
		servers = CaoLiao.settings.get("WebRTC_Servers")
	...
	updateRemoteItems: ->
		...
		for id, peerConnection of @peerConnections
			for remoteStream in peerConnection.getRemoteStreams()
				...
				@log "updateRemoteItems iceConnectionState", peerConnection.iceConnectionState
				switch peerConnection.iceConnectionState
					...
					when 'closed'
						...
						console.log "'disconnected', 'closed'",peerConnection
	...
	onRemoteStatus: (data) ->
		if !@joinWebrtc
			...
			#@log "remoteConnections", remoteConnections
	...
	getPeerConnection: (id) ->
		#@log "@room", ChatRoom.findOne(@room)
		...
		#@log "创建cs getPeerConnection", id, @peerConnections
	...
	getUserMedia: (media, onSuccess, onError=@onError) ->
		...
		getScreen = (audioStream) =>
			if document.cookie.indexOf("caoliaoscreenshare=chrome") is -1 and not window.caoliaoscreenshare?
				refresh = ->
					swal
						type: "warning"
					...
					if isConfirm
						if @navigator is 'chrome'
							chrome.webstore.install undefined, refresh, ->
								window.open('https://chrome.google.com/webstore/detail/caoliao-screen-share/nocfbnnmjnndkbipkabodnheejiegccf')
								refresh()
						else if @navigator is 'firefox'
							window.open('https://addons.mozilla.org/en-GB/firefox/addon/caoliao-screen-share/')
							refresh()
	...
	stopPeerConnection: (id) ->
		...
		#@log "stopPeerConnection", peerConnection
		if not peerConnection? then return


		delete @peerConnections[id]
		console.log "stopPeerConnection peerConnection"
		...
		console.log "ChatRoom.findOne(@room).did", ChatRoom.findOne(@room).did
		Meteor.call "leaveWebrtc", Session.get("openedRoom")
	...
	joinCall: (data={}) ->
		...
		#@log "@canStartUserMedia", @canStartUserMedia()
	...
	onRemoteJoin: (data) ->
		...
		#@log "远程用户已经链接 onRemoteJoin args", data
	...
	###
	onRemoteOffer: (data) ->
		#@log "远程用户已经创建offer onRemoteOffer args", data
		#if @active isnt true then return所有人员可看
	...
	onRemoteDescription: (data) ->
		#@log "远程 sdp 发送过来", data
		#if @active isnt true then return所有人员都可以接受
		if data.to isnt @selfId then return

		peerConnection = @getPeerConnection data.from

		#@log "peerConnection",peerConnection
...
WebRTC = new class
	...
	getInstanceByRoomId: (roomId) ->
		...
		switch subscription.t
			when 'd'
				enabled = CaoLiao.settings.get('WebRTC_Enable_Direct')
			when 'p'
				enabled = CaoLiao.settings.get('WebRTC_Enable_Private')
			when 'c'
				enabled = CaoLiao.settings.get('WebRTC_Enable_Channel')
...
Meteor.startup ->
	Tracker.autorun ->
		if Meteor.userId()
			if Meteor.isCordova
				window.plugins.deviceMedia.get(success, fail);

			CaoLiao.Notifications.onUser 'webrtc', (type, data) =>
